...
* **Gemma-4 Coding & Reasoning Variants** (in `recipes/unsloth/gemma/`):
  * `Gemma-4-12B-NoThinking-qat-MTP.json` — 12B coding variant with reasoning disabled (`--reasoning off`, `--temp 0.1`).
  * `Gemma-4-12B-ThinkingCoder-qat-MTP.json` — 12B coding variant with reasoning enabled (`--reasoning on`, `--temp 0.7`).
  * `Gemma-4-26B-A4B-ThinkingCoder-Q8_0-MTP.json` — 26B coding variant with reasoning enabled (`--reasoning on`, `--temp 0.7`, `--top-k 64`) utilizing the 8-bit quantized weights (`Q8_0`) for AMD Strix Halo targets.
