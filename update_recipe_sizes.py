...
def resolve_checkpoint_files(repo_id: str, variant: str, hf_cache: pathlib.Path) -> List[pathlib.Path]:
    ...
    for f in all_files:
        if f.name.lower().endswith(f"-{variant_lower}.gguf") or f.name.lower().endswith(f"_{variant_lower}.gguf"):
            return [f]
...
def main():
    ...
    for recipe_path in recipe_files:
        ...
        if not isinstance(recipe, dict) or ("recipe" not in recipe and "checkpoints" not in recipe and "checkpoint" not in recipe):
            continue
        ...
        for repo_id, variant in unique_checkpoints:
            if not repo_id and os.path.isabs(variant):
                ...
                if not files:
                    # If any required checkpoint is missing, we cannot determine the full size
                    missing_some = True
                else:
        ...
        if has_main_downloaded:
            ...
            if old_size != size_gib:
                ...
                try:
                    with open(recipe_path, "w", encoding="utf-8") as f:
                        ...
                        f.write("\n")
                    print(f"Updated {recipe_path.relative_to(repo_root)}: {old_size} -> {size_gib} GiB")
                except Exception as e:
                    print(f"Failed to write updated recipe {recipe_path}: {e}")
            else:
                print(f"Recipe {recipe_path.relative_to(repo_root)} is already up-to-date: {size_gib} GiB")
            ...
            if canonical_name in user_models:
                ...
                if old_user_size != size_gib:
                    ...
                    print(f"  Updated user_models.json [{canonical_name}]: {old_user_size} -> {size_gib} GiB")
        else:
            print(f"Skipping {recipe_path.relative_to(repo_root)}: checkpoints not downloaded/found in cache.")
